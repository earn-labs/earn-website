...
export async function GET() {
  try {
    ...
    const totalBurned = Number(formatEther(data));
    // Debug: Log to verify function is actually running
    console.log(`[${new Date().toISOString()}] API called - Total Burned: ${totalBurned}`);

    // Add cache headers to refresh every 5 minutes
    return NextResponse.json({ totalBurned: totalBurned }, { 
      status: 200,
      headers: {
